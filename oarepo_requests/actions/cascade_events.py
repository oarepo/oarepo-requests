...
def _get_requests_with_topic_reference(topic_ref: EntityReference) -> list[dict]:
    return current_requests_service.scan(
        system_identity,
...
def update_topic(request: Request, old_topic: Record, new_topic: Record, uow: UnitOfWork) -> None:
    ...
    the rest of the requests with the same topic
    ...
    for request_from_search in requests_with_topic._results:  # result list links might crash before update of the topic
        ...
        if hasattr(request_type, "topic_change"):
            ...
            ):  # what if we don't change topic but still do some event we want to log,
                # ie. cancelling the request because it does not apply to published record
                payload = {
