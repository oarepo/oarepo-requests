...
class RecordSnapshot(db.Model):
    ...
    @classmethod
    def create(cls, record_uuid: UUID, request_id: UUID, json: dict[str, Any]) -> RecordSnapshot:
        """Create new snapshot."""
        try:
    ...
    def get(cls, record_uuid: UUID) -> RecordSnapshot:
        ...
        try:
            return db.session.query(cls).filter_by(record_uuid=record_uuid).one()  # type: ignore[no-any-return]
    ...
    def get_all(cls, record_uuid: UUID) -> list[RecordSnapshot]:
        ...
        try:
            return db.session.query(cls).filter_by(record_uuid=record_uuid).all()  # type: ignore[no-any-return]
        except NoResultFound as exc:
    ...
    def get_two_latest_snapshots_by_record_uuid(cls, record_uuid: UUID) -> list[RecordSnapshot]:
        ...
        try:
            return db.session.query(cls).filter_by(record_uuid=record_uuid).order_by(cls.created.desc()).limit(2).all()  # type: ignore[no-any-return]
        except NoResultFound as exc:
            raise ValueError(f"No record snapshot for record uuid {record_uuid}") from exc
