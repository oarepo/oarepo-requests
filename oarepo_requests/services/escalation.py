...
log = logging.getLogger("oarepo.requests.escalation")


@unit_of_work()
def escalate_request(
    request: Request, escalation: WorkflowRequestEscalation, uow=None
) -> None:
    """Escalate single request and commit the change to the database."""
    log.info(f"Escalating request {request.id}")
    resolved_topic = request.topic.resolve()
    receiver = RecipientEntityReference(
        request_or_escalation=escalation, record=resolved_topic
    )
    ...
    if new_receiver_str != old_receiver_str:
        log.info(
            f"Request {request.id} receiver changed from {old_receiver_str} to {new_receiver_str}"
        )
        ...
        log.info(f"Notification mail sent to {new_receiver_str}")
