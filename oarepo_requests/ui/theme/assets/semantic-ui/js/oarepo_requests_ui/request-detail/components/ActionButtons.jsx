  ...
  return (
    <>
      {request.links?.actions?.submit && (
        <ConfirmModal
          request={request}
          requestModalHeader={i18next.t("Submit") + " " + i18next.t("request")}
          handleSubmit={(values) =>
            callApi(request.links.actions.submit, "POST", values)
          }
          triggerButton={
            <Button
              compact
      ...
      {request.links?.actions?.accept && (
        <ConfirmModal
          request={request}
          requestModalHeader={i18next.t("Accept") + " " + i18next.t("request")}
          handleSubmit={(values) =>
      ...
      {request.links?.actions?.cancel && (
        <ConfirmModal
          request={request}
          requestModalHeader={i18next.t("Cancel") + " " + i18next.t("request")}
          handleSubmit={(values) =>
          ...
          submitButton={
            <Button
              ...
              compact
              icon="close"
              content={i18next.t("Cancel") + " " + i18next.t("request")}
            />
          }
        />
      )}
      {request.links?.actions?.decline && (
        <ConfirmModal
          request={request}
          requestModalHeader={i18next.t("Decline") + " " + i18next.t("request")}
          handleSubmit={(values) =>
