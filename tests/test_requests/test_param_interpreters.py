...
def _init(
    ...
    draft1 = create_draft_via_resource(user1_client, custom_workflow="different_recipients")
    draft2 = create_draft_via_resource(user2_client, custom_workflow="different_recipients")

    submit_response_user1 = submit_request(user1_client, draft1, "publish_draft") # recipient should be 2
    submit_response_user2 = submit_request(user2_client, draft2, "another_topic_updating") # should be 1
...
def test_receiver_param_interpreter(
    ...
    user1_client, user2_client = _init(users, logged_client, create_draft_via_resource, submit_request_by_link, urls)
    search_receiver_only = user2_client.get(f'{urls["BASE_URL_REQUESTS"]}?assigned=true') # creator of 1 and recipient of 2
...
def test_open_param_interpreter(
    ...
    submit_response_user1 = submit_request_by_link(user1_client, draft1, "publish_draft")
    submit_response_user2 = submit_request_by_link(user1_client, draft2, "publish_draft")
    ...
    publish = user2_client.post(link_api2testclient(read.json['expanded']['requests'][0]['links']['actions']['accept']))
