...
def test_publish(
    ...
    publish = logged_client_post(receiver, "post",
        link_api2testclient(record.json["requests"][0]["links"]["actions"]["accept"])
    )
    record = logged_client_post(receiver, "get",f"{urls['BASE_URL']}{draft2.json['id']}/draft")
    ...
    resp_request_create = logged_client_post(creator, "post",
        urls["BASE_URL_REQUESTS"], json=publish_request_data_function(receiver.id, draft3.json["id"])
    )
    resp_request_submit = logged_client_post(creator, "post",
        link_api2testclient(resp_request_create.json["links"]["actions"]["submit"])
    )
    record = logged_client_post(creator, "get",f"{urls['BASE_URL']}{draft3.json['id']}/draft")
