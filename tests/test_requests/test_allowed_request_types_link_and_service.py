...
def test_allowed_request_types_on_draft_service(
    ...
    allowed_request_types = thesis_requests_service.get_applicable_request_types_for_draft(
...
def publish_record(creator_client, urls, publish_request_data_function, draft1, receiver_client):
...
def test_allowed_request_types_on_published_resource(
    ...
    draft1 = create_draft_via_resource(creator_client)
    published1 = publish_record(creator_client, urls, publish_request_data_function, draft1, receiver_client)

    applicable_requests_link = published1["links"]["applicable-requests"]
    assert applicable_requests_link == f'https://127.0.0.1:5000/api/thesis/{published1["id"]}/requests/applicable'
    allowed_request_types = creator_client.get(link_api2testclient(applicable_requests_link))
    assert sorted(allowed_request_types.json["hits"]["hits"], key=lambda x: x['type_id']) == [{'links': {
        'actions': {'create': f'https://127.0.0.1:5000/api/thesis/{published1["id"]}/requests/delete_published_record'}},
