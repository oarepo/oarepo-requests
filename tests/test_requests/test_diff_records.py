...
def test_new_record(users, record_service, default_record_with_workflow_json, search_clear):
...
def test_diff_after_publish_is_denied(
    ...
    record = receiver_client.get(f"{link2testclient(draft2['links']['self'])}?expand=true")
...
def test_new_version_diff(
    ...
    new_version_direct = creator_client.post(
        link2testclient(record1["links"]["versions"]),
    )
    ...
    new_draft = creator_client.get(link2testclient(new_draft["links"]["self"])).json
    new_draft["metadata"]["title"] = "new title"

    assert creator_client.put(link2testclient(new_draft["links"]["self"]), json=new_draft).status_code == 200
...
def test_edited_metadata_diff(
    ...
    new_version_direct = creator_client.post(link2testclient(record1["links"]["versions"]))
    assert new_version_direct.status_code == 403

    resp_request_submit = submit_request_on_record(creator.identity, record1_id, "edit_published_record")
    # is request accepted and closed?
    request = creator_client.get(
        f"{urls['BASE_URL_REQUESTS']}{resp_request_submit['id']}",
    ).json
...
def test_request_active_diff(
    users, default_record_with_workflow_json, submit_request_on_draft, record_service, logged_client, urls, search_clear
):
    from oarepo_requests.proxies import current_requests_service
    ...
    current_requests_service.execute_action(receiver.identity, request.id, "accept")
